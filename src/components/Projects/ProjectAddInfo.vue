      ...
      <div class="projectAdd__info-form">
        ...
        <form>
          <div class="projectAdd__info-group">
            ...
            >
              Название проекта должен содержать минимум {{ validator.title.$params.minLength.min }} символов. Сейчас он
              {{ getInfoProject.project.title.length }}
            </small>
          </div>
          <div class="projectAdd__info-group">
            ...
            >
              Название проекта должен содержать минимум {{ validator.description.$params.minLength.min }} символов.
              Сейчас он {{ getInfoProject.project.description.length }}
            </small>
          </div>
          <div class="projectAdd__info-group">
            <Datepicker :language="lang" :disabled-dates="disabledDates" id="deadline"
                        v-model="getInfoProject.project.deadline" :format="format"/>
  ...
  methods: {
    photoSubmit(e) {
      ...
      if (e.target.files[0].size >= 1500000){
        this.$store.commit('setNotification', 'file-size-PostTooLargeException')
        return
      }
